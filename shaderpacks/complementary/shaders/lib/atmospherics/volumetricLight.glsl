...
vec3 getVolumetricRays(float pixeldepth0, float pixeldepth1, vec3 color, float dither) {
	...
	float visibilityFactor = min(eBS + isEyeInWater, 1.0);
	#else
	float visibilityFactor = 0.0;
	#endif
	if (eyeAltitude < 2.0) visibilityFactor *= clamp((eyeAltitude-1.0), 0.65, 1.0);
	...
	if (visibility > 0.0){
		...
		for(int i = 0; i < 7; i++) {
			...
			if (length(worldposition.xy * 2.0 - 1.0) < 1.0){
				...
				else if (isEyeInWater == 1.0) sample *= watercol * watercol * 250;
